...
static
void
dsp_fast_load_alu_operands(uint32_t const pc_)
{
  ITAG_t inst;
...
static
bool
dsp_fast_branch_taken_at(uint32_t const        pc_,
                         dsp_alu_flags_t const *flags_,
                         int const             *fExact_)
{
  ITAG_t branch;

  branch.raw = DSP.NMem[pc_];
  DSP.dregs.PC = pc_ + 1;
                        ...
                        bool            *work_)
{
  ...
  branch_final.raw = DSP.NMem[base + 3];
  jump_common.raw  = DSP.NMem[base + 6];
  if((branch_down.cif.BCH_ADDR != ((base + 0x07) & 0x3FF)) ||
     (branch_final.cif.BCH_ADDR != ((base + 0x0E) & 0x3FF)) ||
     (jump_common.cif.BCH_ADDR != ((base + 0x0C) & 0x3FF)))
    return false;
  ...
  *Y_              = y;
  ...
  DSP.dregs.PC = base + 4;
  if(1 & DSP.BRCONDTAB[branch_final.br.bits][*fExact_ + ((flags_->raw * 0x10080402) >> 24)])
    {
      DSP.dregs.PC = branch_final.cif.BCH_ADDR;
      goto final_tail;
    }

  mov.raw     = DSP.NMem[base + 4];
  operand.raw = DSP.NMem[base + 5];
...
down_step:
  operand.raw = DSP.NMem[base + 8];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 7]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 7]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  mov.raw     = DSP.NMem[base + 10];
  operand.raw = DSP.NMem[base + 11];
...
common_move:
  mov.raw     = DSP.NMem[base + 12];
  operand.raw = DSP.NMem[base + 13];
...
final_tail:
  operand.raw = DSP.NMem[base + 15];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 14]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 14]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + 16;
  ...
  *Y_              = y;
  ...
  operand.raw = DSP.NMem[base + 17];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 16]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 16]].BS;
  DSP.flags.WRITEBACK = 0;
                      ...
                      bool            *work_)
{
  ...
  branch.raw = DSP.NMem[base + 14];
  jump.raw   = DSP.NMem[base + 18];
  if((branch.cif.BCH_ADDR != ((base + 0x13) & 0x3FF)) ||
     (jump.cif.BCH_ADDR != ((base + 0x15) & 0x3FF)))
    return false;
  ...
  y = ((uint32_t)(uint16_t)DSP.flags.ALU1 << 16);
  dsp_fast_set_product_flags(y,flags_,fExact_);
  *Y_ = (uint32_t)(((int32_t)y >> 8) & ALUSIZEMASK);
  ...
  *Y_ = y << 8;
  ...
  src.raw = DSP.NMem[base + 6];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 5]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 5]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  src.raw = DSP.NMem[base + 10];
  dst.raw = DSP.NMem[base + 11];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 9]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 9]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  src.raw = DSP.NMem[base + 13];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 12]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 12]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  src.raw = DSP.NMem[base + 16];
  dst.raw = DSP.NMem[base + 17];
...
tail_move:
  move.raw    = DSP.NMem[base + 19];
  operand.raw = DSP.NMem[base + 20];

  DSP.dregs.PC = base + DSP_PULSE_LFO_23_WORDS;
  val = dsp_read(operand.nrof.OP_ADDR);
  if(operand.nrof.DI)
    val = dsp_read(val);
  addr = move.cif.BCH_ADDR;
  if(move.nrof.DI)
    addr = dsp_read(addr);
  dsp_write(addr,val);
                   ...
                   bool            *work_)
{
  ...
  branch_initial.raw = DSP.NMem[base + 3];
  jump_initial.raw   = DSP.NMem[base + 7];
  branch_tail.raw    = DSP.NMem[base + 20];
  jump_tail.raw      = DSP.NMem[base + 24];
  if((branch_initial.cif.BCH_ADDR != ((base + 0x09) & 0x3FF)) ||
     (jump_initial.cif.BCH_ADDR != ((base + 0x0C) & 0x3FF)) ||
     (branch_tail.cif.BCH_ADDR != ((base + 0x1A) & 0x3FF)) ||
     (jump_tail.cif.BCH_ADDR != ((base + 0x1C) & 0x3FF)))
    return false;
  ...
  *Y_              = y;
  ...
  DSP.dregs.PC = base + 4;
  if(1 & DSP.BRCONDTAB[branch_initial.br.bits][*fExact_ + ((flags_->raw * 0x10080402) >> 24)])
    {
      DSP.dregs.PC = branch_initial.cif.BCH_ADDR;
      goto high_pulse;
    }
  ...
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 5]].BS;
  DSP.flags.WRITEBACK = 0;
  DSP.flags.ALU1 = (uint16_t)(operand.iof.IMMEDIATE << (operand.iof.JUSTIFY & 3));
  ...
  *Y_              = y << 1;
  ...
  DSP.dregs.PC = jump_initial.cif.BCH_ADDR;
  goto middle;
...
middle:
  ...
  *Y_              = y;
  ...
  src.raw = DSP.NMem[base + 19];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 18]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 18]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  DSP.dregs.PC = base + 21;
  if(1 & DSP.BRCONDTAB[branch_tail.br.bits][*fExact_ + ((flags_->raw * 0x10080402) >> 24)])
    {
      DSP.dregs.PC = branch_tail.cif.BCH_ADDR;
      goto high_tail;
    }

  operand.raw = DSP.NMem[base + 23];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 22]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 22]].BS;
  DSP.flags.WRITEBACK = 0;
  DSP.flags.ALU1 = (uint16_t)(operand.iof.IMMEDIATE << (operand.iof.JUSTIFY & 3));
  ...
  *Y_ = y;
  ...
  DSP.dregs.PC = jump_tail.cif.BCH_ADDR;
  goto final_tail;

high_tail:
  operand.raw = DSP.NMem[base + 27];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 26]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 26]].BS;
  DSP.flags.WRITEBACK = 0;
  DSP.flags.ALU1 = (uint16_t)(operand.iof.IMMEDIATE << (operand.iof.JUSTIFY & 3));
...
final_tail:
  src.raw = DSP.NMem[base + 29];
  dst.raw = DSP.NMem[base + 30];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 28]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 28]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + 30;
  DSP.flags.WRITEBACK = src.nrof.OP_ADDR;
  DSP.flags.MULT1 = dsp_read(DSP.flags.WRITEBACK);

  DSP.dregs.PC = base + DSP_PULSER_33_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                       ...
                       bool            *work_)
{
  ...
  *Y_              = y;
  ...
  rand_dst.raw = DSP.NMem[base + 4];

  DSP.dregs.PC = base + 6;
  val = dsp_read(0x0EA);
  dsp_write(rand_dst.cif.BCH_ADDR,val);

  DSP.dregs.PC = base + 7;

multiply_tail:
  src1.raw = DSP.NMem[base + 8];
  src2.raw = DSP.NMem[base + 9];
  dst.raw  = DSP.NMem[base + 10];
  ...
  DSP.dregs.PC = base + DSP_RANDOMHOLD_13_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                     ...
                     bool            *work_)
{
  ...
  branch.raw = DSP.NMem[base + 3];
  if(branch.cif.BCH_ADDR != ((base + 0x08) & 0x3FF))
    return false;
  ...
  dst.raw = DSP.NMem[base + 1];
  src.raw = DSP.NMem[base + 2];
  ...
  *Y_              = y;
  ...
  move.raw    = DSP.NMem[base + 4];
  operand.raw = DSP.NMem[base + 5];
  DSP.dregs.PC = base + 6;
  val = dsp_read(operand.nrof.OP_ADDR);
  dsp_write(move.cif.BCH_ADDR,val);

  move.raw = DSP.NMem[base + 6];
  DSP.dregs.PC = base + 8;
  val = dsp_read(0x0EA);
  dsp_write(move.cif.BCH_ADDR,val);

filter_tail:
  dst.raw = DSP.NMem[base + 9];
  ...
  y = *Y_;
  dsp_fast_set_product_flags(y,flags_,fExact_);
  *Y_ = (uint32_t)((y >> 1) & ALUSIZEMASK);

  if(DSP.flags.WRITEBACK)
    dsp_write(DSP.flags.WRITEBACK,((int32_t)*Y_) >> 16);

  src1.raw = DSP.NMem[base + 11];
  src2.raw = DSP.NMem[base + 12];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 10]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 10]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[base + 14];
  src2.raw = DSP.NMem[base + 15];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 13]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 13]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[base + 17];
  dst.raw  = DSP.NMem[base + 18];
  ...
  DSP.dregs.PC = base + DSP_REDNOISE_21_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                    ...
                    bool            *work_)
{
  ...
  branch.raw      = DSP.NMem[base + 3];
  branch_tail.raw = DSP.NMem[base + 10];
  if((branch.cif.BCH_ADDR != ((base + 0x08) & 0x3FF)) ||
     (branch_tail.cif.BCH_ADDR != ((base + 0x11) & 0x3FF)))
    return false;
  ...
  DSP.flags.ALU2 = (uint16_t)(src2.iof.IMMEDIATE << (src2.iof.JUSTIFY & 3));
  DSP.flags.WRITEBACK = 0;
...
semaphore_tail:
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[base + 12];
  src2.raw = DSP.NMem[base + 13];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 11]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 11]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[base + 15];
  src2.raw = DSP.NMem[base + 16];
...
final_tail:
  src1.raw = DSP.NMem[base + 18];
  src2.raw = DSP.NMem[base + 19];
  dst.raw  = DSP.NMem[base + 20];
                       ...
                       bool            *work_)
{
  ...
  if(pc == (base + 0x61))
    {
      DSP.dregs.PC = base + DSP_SAMPLER3D_100_WORDS;
      return true;
    }
  if(pc == (base + 0x51))
    goto tail_51;
...
sample_setup:
  src.raw = DSP.NMem[setup + 0];
  DSP.dregs.PC = setup + 1;
  DSP.RBASEx4 = ((src.cif.BCH_ADDR & 0x3F) << 2);

  mov.raw = DSP.NMem[setup + 1];
  DSP.dregs.PC = setup + 2;
  val = dsp_operand_load1();
  addr = DSP.REGCONV[DSP.REGi][mov.r2of.R1] ^ DSP.RBASEx4;
  if(mov.r2of.R1_DI)
    addr = dsp_read(addr);
  ...
  mov.raw = DSP.NMem[setup + 3];
  DSP.dregs.PC = setup + 4;
  val = dsp_operand_load1();
  addr = DSP.REGCONV[DSP.REGi][mov.r2of.R1] ^ DSP.RBASEx4;
  if(mov.r2of.R1_DI)
    addr = dsp_read(addr);
  ...
  jsr.raw = DSP.NMem[setup + 5];
  *RBSR_ = setup + 0x06;
  DSP.dregs.PC = jsr.cif.BCH_ADDR;
  if(DSP.dregs.PC != (base + 0x3A))
    return true;

sample_tail:
  dsp_fast_load_alu_operands(base + 0x3A);
  ...
  *Y_              = y;
  ...
  branch.raw = DSP.NMem[base + 0x3C];
  DSP.dregs.PC = base + 0x3D;
  if(1 & DSP.BRCONDTAB[branch.br.bits][*fExact_ + ((flags_->raw * 0x10080402) >> 24)])
    {
      DSP.dregs.PC = branch.cif.BCH_ADDR;
      if(DSP.dregs.PC == (base + 0x51))
        goto tail_51;
      return true;
    }

  branch.raw = DSP.NMem[base + 0x3D];
  DSP.dregs.PC = base + 0x3E;
  if(1 & DSP.BRCONDTAB[branch.br.bits][*fExact_ + ((flags_->raw * 0x10080402) >> 24)])
    {
      DSP.dregs.PC = branch.cif.BCH_ADDR;
      if(DSP.dregs.PC == (base + 0x44))
        goto tail_44;
      return true;
    }
...
tail_44:
  dsp_fast_load_alu_operands(base + 0x44);
  ...
  *Y_              = y;
  ...
  mov.raw = DSP.NMem[base + 0x47];
  DSP.dregs.PC = base + 0x48;
  val = dsp_operand_load1();
  addr = DSP.REGCONV[DSP.REGi][mov.r2of.R1] ^ DSP.RBASEx4;
  if(mov.r2of.R1_DI)
    addr = dsp_read(addr);
  ...
  mov.raw = DSP.NMem[base + 0x49];
  DSP.dregs.PC = base + 0x4A;
  val = dsp_operand_load1();
  addr = DSP.REGCONV[DSP.REGi][mov.r2of.R1] ^ DSP.RBASEx4;
  if(mov.r2of.R1_DI)
    addr = dsp_read(addr);
...
tail_51:
  dsp_fast_load_alu_operands(base + 0x51);
  ...
  *Y_              = y;
  ...
  dsp_fast_load_alu_operands(base + 0x54);
  ...
  *Y_              = y;
  ...
  mov.raw = DSP.NMem[base + 0x57];
  DSP.dregs.PC = base + 0x58;
  val = dsp_operand_load1();
  addr = DSP.REGCONV[DSP.REGi][mov.r2of.R1] ^ DSP.RBASEx4;
  if(mov.r2of.R1_DI)
    addr = dsp_read(addr);
  ...
  mov.raw = DSP.NMem[base + 0x59];
  DSP.dregs.PC = base + 0x5A;
  val = dsp_operand_load1();
  addr = DSP.REGCONV[DSP.REGi][mov.r2of.R1] ^ DSP.RBASEx4;
  if(mov.r2of.R1_DI)
    addr = dsp_read(addr);
...
tail_vector:
  dsp_fast_load_alu_operands(tail);
  ...
  *Y_              = y;
  ...
  dsp_fast_load_alu_operands(tail + 2);
  ...
  *Y_              = y;
  ...
  if(DSP.dregs.PC == (base + 0x61))
    DSP.dregs.PC = base + DSP_SAMPLER3D_100_WORDS;
  return true;

channel_block:
  dst.raw = DSP.NMem[block + 0];
  DSP.dregs.PC = block + 1;
  val = dsp_operand_load1();
  addr = dst.cif.BCH_ADDR;
  if(dst.nrof.DI)
    addr = dsp_read(addr);
  dsp_write(addr,val);

  dsp_fast_load_alu_operands(block + 2);
  ...
  *Y_ = y;
  ...
  dsp_fast_load_alu_operands(block + 5);
  ...
  *Y_              = y;
  ...
  dsp_fast_load_alu_operands(block + 8);
  ...
  *Y_              = y;
  ...
  dst.raw = DSP.NMem[block + 12];
  DSP.dregs.PC = block + 13;
  val = dsp_operand_load1();
  addr = dst.cif.BCH_ADDR;
  if(dst.nrof.DI)
    addr = dsp_read(addr);
  ...
  dst.raw = DSP.NMem[block + 14];
  DSP.dregs.PC = block + 15;
  val = dsp_operand_load1();
  addr = dst.cif.BCH_ADDR;
  if(dst.nrof.DI)
    addr = dsp_read(addr);
  ...
  dst.raw = DSP.NMem[block + 16];
  DSP.dregs.PC = block + 17;
  val = dsp_operand_load1();
  addr = dst.cif.BCH_ADDR;
  if(dst.nrof.DI)
    addr = dsp_read(addr);
  ...
  dsp_fast_load_alu_operands(block + 18);
                       ...
                       bool            *work_)
{
  ...
  jsr.raw = DSP.NMem[base + 28];
  if((jsr.cif.BCH_ADDR >= base) &&
     (jsr.cif.BCH_ADDR < (base + DSP_SAMPLERENV_36_WORDS)) &&
     (jsr.cif.BCH_ADDR != (base + 0x1D)))
    return false;
  ...
  *Y_              = y;
  ...
  src2.raw = DSP.NMem[base + 6];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 4]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 4]].BS;
  DSP.flags.WRITEBACK = 0;
...
copy_path:
  ...
  *Y_              = y;
  ...
  dst.raw = DSP.NMem[base + 19];
  imm.raw = DSP.NMem[base + 20];
  ...
  dst.raw = DSP.NMem[base + 21];
  src.raw = DSP.NMem[base + 22];

  DSP.dregs.PC = base + 23;
  val = dsp_read(src.nrof.OP_ADDR);
  if(src.nrof.DI)
    val = dsp_read(val);
  addr = dst.cif.BCH_ADDR;
...
sample_setup:
  src.raw = DSP.NMem[base + 23];
  DSP.dregs.PC = base + 24;
  DSP.RBASEx4 = ((src.cif.BCH_ADDR & 0x3F) << 2);

  mov.raw = DSP.NMem[base + 24];
  imm.raw = DSP.NMem[base + 25];

  DSP.dregs.PC = base + 26;
  val = (uint16_t)(imm.iof.IMMEDIATE << (imm.iof.JUSTIFY & 3));
  addr = DSP.REGCONV[DSP.REGi][mov.r2of.R1] ^ DSP.RBASEx4;
  if(mov.r2of.R1_DI)
    addr = dsp_read(addr);
  ...
  mov.raw = DSP.NMem[base + 26];
  src.raw = DSP.NMem[base + 27];
  ...
  *RBSR_ = base + 0x1D;
  DSP.dregs.PC = jsr.cif.BCH_ADDR;
  if(DSP.dregs.PC != (base + 0x1D))
    return true;

sample_tail:
  src.raw = DSP.NMem[base + 30];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 29]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 29]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + 31;
  DSP.flags.WRITEBACK = src.nrof.OP_ADDR;
  DSP.flags.MULT1 = dsp_read(DSP.flags.WRITEBACK);
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src.raw = DSP.NMem[base + 32];
  dst.raw = DSP.NMem[base + 33];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 31]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 31]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  DSP.dregs.PC = base + DSP_SAMPLERENV_36_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                       ...
                       bool            *work_)
{
  ...
  jsr.raw = DSP.NMem[base + 12];
  if((jsr.cif.BCH_ADDR >= base) &&
     (jsr.cif.BCH_ADDR < (base + DSP_SAMPLERMOD_18_WORDS)) &&
     (jsr.cif.BCH_ADDR != (base + 0x0D)))
    return false;
  ...
  src.raw = DSP.NMem[base + 2];
  imm.raw = DSP.NMem[base + 3];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 1]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 1]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  imm.raw  = DSP.NMem[base + 8];
  src.raw  = DSP.NMem[base + 9];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 7]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 7]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.flags.ALU1 = (uint16_t)(imm.iof.IMMEDIATE << (imm.iof.JUSTIFY & 3));

  DSP.dregs.PC = base + 10;
  DSP.flags.WRITEBACK = DSP.REGCONV[DSP.REGi][src.r2of.R1] ^ DSP.RBASEx4;
  if(src.r2of.R1_DI)
  ...
  *RBSR_ = base + 0x0D;
  DSP.dregs.PC = jsr.cif.BCH_ADDR;
  if(DSP.dregs.PC != (base + 0x0D))
    return true;

sample_tail:
  src.raw  = DSP.NMem[base + 14];
  src2.raw = DSP.NMem[base + 15];
  ...
  DSP.dregs.PC = base + DSP_SAMPLERMOD_18_WORDS;
  DSP.flags.WRITEBACK = src2.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
...
tail:
  src1.raw = DSP.NMem[base + 25];
  src2.raw = DSP.NMem[base + 26];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 24]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 24]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[base + 28];
  src2.raw = DSP.NMem[base + 29];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 27]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 27]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + 29;
  DSP.flags.WRITEBACK = src1.nrof.OP_ADDR;
  DSP.flags.MULT1 = dsp_read(DSP.flags.WRITEBACK);
  ...
  *Y_ = y;
  ...
  src.raw = DSP.NMem[base + 31];
  dst.raw = DSP.NMem[base + 32];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 30]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 30]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + 32;
  DSP.flags.WRITEBACK = src.nrof.OP_ADDR;
  DSP.flags.MULT1 = dsp_read(DSP.flags.WRITEBACK);

  DSP.dregs.PC = base + DSP_SAWENV_35_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                             ...
                             bool            *work_)
{
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[pc + 4];
  src2.raw = DSP.NMem[pc + 5];
  dst.raw  = DSP.NMem[pc + 6];
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[pc + 9];
  src2.raw = DSP.NMem[pc + 10];
  dst.raw  = DSP.NMem[pc + 11];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 8]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 8]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 13];
  src2.raw = DSP.NMem[pc + 14];
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 17];
  dst.raw  = DSP.NMem[pc + 18];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 16]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 16]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 20];
  dst.raw  = DSP.NMem[pc + 21];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 19]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 19]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 23];
  src2.raw = DSP.NMem[pc + 24];
  dst.raw  = DSP.NMem[pc + 25];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 22]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 22]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  DSP.dregs.PC = pc + DSP_SAWFILTEREDNOISE_29_WORDS;
                           ...
                           bool            *work_)
{
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[pc + 8];
  src2.raw = DSP.NMem[pc + 9];
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[pc + 11];
  dst.raw  = DSP.NMem[pc + 12];
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 15];
  src2.raw = DSP.NMem[pc + 16];
  dst.raw  = DSP.NMem[pc + 17];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 14]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 14]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 19];
  src2.raw = DSP.NMem[pc + 20];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 18]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 18]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 26];
  dst.raw  = DSP.NMem[pc + 27];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 25]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 25]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 29];
  src2.raw = DSP.NMem[pc + 30];
  dst.raw  = DSP.NMem[pc + 31];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 28]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 28]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  DSP.dregs.PC = pc + DSP_SAWFILTEREDSAW_35_WORDS;
                    ...
                    bool            *work_)
{
  ...
  *Y_              = y;
  ...
  DSP.dregs.PC = pc + DSP_SAWTOOTH_8_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                   ...
                   bool            *work_)
{
  ...
  *Y_              = y;
  ...
  jump.raw = DSP.NMem[base + 7];
  DSP.dregs.PC = jump.cif.BCH_ADDR;

  return true;

tail_move:
  tail_dst.raw = DSP.NMem[base + 8];
  tail_src.raw = DSP.NMem[base + 9];

  DSP.dregs.PC = base + DSP_SQUARE_12_WORDS;
  val = dsp_read(tail_src.nrof.OP_ADDR);
  dsp_write(tail_dst.cif.BCH_ADDR,val);
                       ...
                       bool            *work_)
{
  ...
  *Y_              = y;
  ...
  jump.raw = DSP.NMem[base + 11];
  DSP.dregs.PC = jump.cif.BCH_ADDR;

  return true;

tail_move:
  tail_dst.raw = DSP.NMem[base + 12];
  tail_src.raw = DSP.NMem[base + 13];

  DSP.dregs.PC = base + DSP_SQUARE_LFO_16_WORDS;
  val = dsp_read(tail_src.nrof.OP_ADDR);
  dsp_write(tail_dst.cif.BCH_ADDR,val);
                        ...
                        bool            *work_)
{
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 10];
  src2.raw = DSP.NMem[pc + 11];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 9]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 9]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  DSP.dregs.PC = pc + DSP_SUBMIXER2X2_20_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                     ...
                     bool            *work_)
{
  ...
  *Y_ = y;
  ...
  src2.raw = DSP.NMem[pc + 6];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 4]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 4]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 14];
  src2.raw = DSP.NMem[pc + 15];
  dst.raw  = DSP.NMem[pc + 16];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 13]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 13]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  DSP.dregs.PC = pc + DSP_SVFILTER_19_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                     ...
                     bool            *work_)
{
  ...
  src1.raw = DSP.NMem[pc + 2];
  src2.raw = DSP.NMem[pc + 3];
  dst.raw  = DSP.NMem[pc + 4];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 1]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 1]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 6];
  src2.raw = DSP.NMem[pc + 7];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 5]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 5]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  src1.raw = DSP.NMem[pc + 16];
  src2.raw = DSP.NMem[pc + 17];
  dst.raw  = DSP.NMem[pc + 18];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[pc + 15]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[pc + 15]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  DSP.dregs.PC = pc + DSP_SVFILTER_22_WORDS;
                   ...
                   bool            *work_)
{
  ...
  *Y_              = y;
  ...
  DSP.dregs.PC = base + 8;
  addr = src.nrof.OP_ADDR;
  val = dsp_read(addr);
  if(src.nrof.DI)
    val = dsp_read(val);
  dsp_write(dst.cif.BCH_ADDR,val);

tail_subtract:
  src1.raw = DSP.NMem[base + 9];
  src2.raw = DSP.NMem[base + 10];
  wb.raw   = DSP.NMem[base + 11];
  ...
  DSP.dregs.PC = base + 12;
  DSP.flags.WRITEBACK = wb.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
...
tail_subtract:
  ...
  DSP.dregs.PC = base + 10;
  DSP.flags.ALU1 = (uint16_t)(imm.iof.IMMEDIATE << (imm.iof.JUSTIFY & 3));
  DSP.flags.WRITEBACK = DSP.flags.ALU1;
                     ...
                     bool            *work_)
{
  ...
  *Y_              = y;
  ...
  flags_->carry    = SUB_CFLAG(a,0x1000,y);
  flags_->overflow = SUB_VFLAG(a,0x1000,y);
  flags_->zero     = ((y & 0xFFFF0000) ? 0 : 1);
  flags_->negative = ((y >> 31) ? 1 : 0);
  *fExact_         = ((y & 0x0000F000) ? 0 : 1);
  *Y_              = (y << 1);
...
multiply_tail:
  src.raw = DSP.NMem[base + 11];
  dst.raw = DSP.NMem[base + 12];
  ...
  DSP.dregs.PC = base + DSP_TRIANGLE_15_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                      ...
                      bool            *work_)
{
  ...
  dsp_fast_execute_alu_at(base + 4,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 9,flags_,fExact_))
    return true;

  dsp_fast_execute_alu_at(base + 10,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 12,Y_,flags_,fExact_);
  dsp_fast_execute_movereg_at(base + 17);
  dsp_fast_execute_alu_at(base + 19,Y_,flags_,fExact_);
  dsp_fast_execute_move_at(base + 25);
  dsp_fast_execute_move_at(base + 27);
  ...
  dsp_fast_execute_move_at(base + 37);
  dsp_fast_execute_move_at(base + 39);
  dsp_fast_control_target_at(base + 41);
  return true;

path_42:
  if(dsp_fast_branch_taken_at(base + 42,flags_,fExact_))
    return true;

  dsp_fast_execute_move_at(base + 43);
  dsp_fast_execute_move_at(base + 45);
  dsp_fast_execute_move_at(base + 47);
  dsp_fast_execute_move_at(base + 49);
  dsp_fast_control_target_at(base + 52);
  return true;

path_53:
  dsp_fast_execute_alu_at(base + 53,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 55,flags_,fExact_))
    return true;
...
path_68:
  dsp_fast_execute_alu_at(base + 68,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 72,flags_,fExact_))
    return true;

  dsp_fast_execute_move_at(base + 73);
  dsp_fast_execute_alu_at(base + 77,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 81,flags_,fExact_))
    return true;

  dsp_fast_control_target_at(base + 103);
  return true;

path_104:
  dsp_fast_execute_alu_at(base + 104,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 108,flags_,fExact_))
    return true;

  dsp_fast_execute_move_at(base + 109);

path_111:
  dsp_fast_execute_alu_at(base + 111,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 115,flags_,fExact_))
    return true;

  dsp_fast_execute_move_at(base + 116);

path_118:
  dsp_fast_execute_alu_at(base + 118,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 122,flags_,fExact_))
    return true;

  dsp_fast_execute_move_at(base + 123);

path_127:
  dsp_fast_execute_movereg_at(base + 127);
  dsp_fast_execute_movereg_at(base + 129);
  dsp_fast_execute_movereg_at(base + 131);
  dsp_fast_execute_movereg_at(base + 133);
  dsp_fast_execute_alu_at(base + 135,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 139,Y_,flags_,fExact_);
...
path_144:
  ...
  dsp_fast_execute_movereg_at(base + 149);
  dsp_fast_execute_move_at(base + 152);

path_154:
  dsp_fast_execute_alu_at(base + 154,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 158,flags_,fExact_))
    return true;

  dsp_fast_execute_alu_at(base + 159,Y_,flags_,fExact_);

path_162:
  dsp_fast_execute_alu_at(base + 162,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 166,flags_,fExact_))
    return true;
...
path_172:
  ...
  dsp_fast_execute_alu_at(base + 180,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 182,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 185,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 187,flags_,fExact_))
    return true;
...
path_191:
  ...
  dsp_fast_execute_alu_at(base + 195,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 197,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 202,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 205,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 206);
  dsp_fast_control_target_at(base + 208);
  return true;

path_209:
  dsp_fast_execute_alu_at(base + 209,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 212,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 213);

path_215:
  dsp_fast_execute_alu_at(base + 215,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 219,Y_,flags_,fExact_);
  dsp_fast_execute_movereg_at(base + 221);
  dsp_fast_execute_move_at(base + 223);
  dsp_fast_execute_move_at(base + 225);
  dsp_fast_execute_move_at(base + 227);
  dsp_fast_execute_move_at(base + 229);
  dsp_fast_execute_move_at(base + 231);
                      ...
                      bool            *work_)
{
  ...
  dsp_fast_execute_movereg_at(base + 14);
  dsp_fast_execute_alu_at(base + 16,Y_,flags_,fExact_);
  dsp_fast_execute_move_at(base + 22);
  dsp_fast_execute_move_at(base + 24);
  dsp_fast_execute_move_at(base + 26);
  dsp_fast_execute_move_at(base + 28);
  ...
  dsp_fast_control_target_at(base + 42);
  return true;

path_43:
  if(dsp_fast_branch_taken_at(base + 43,flags_,fExact_))
    return true;

  dsp_fast_execute_move_at(base + 44);
  dsp_fast_execute_move_at(base + 46);
  dsp_fast_execute_move_at(base + 48);
  dsp_fast_control_target_at(base + 51);
  return true;

path_52:
  dsp_fast_execute_movereg_at(base + 52);
  dsp_fast_execute_alu_at(base + 54,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 58,flags_,fExact_))
    return true;

path_59:
  dsp_fast_execute_alu_at(base + 59,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 62,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 63);
...
path_71:
  dsp_fast_execute_alu_at(base + 71,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 74,flags_,fExact_))
    return true;

  dsp_fast_execute_move_at(base + 75);
  dsp_fast_execute_alu_at(base + 78,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 82,Y_,flags_,fExact_);
  dsp_fast_control_target_at(base + 86);
  return true;

path_87:
  dsp_fast_execute_move_at(base + 87);

path_89:
  dsp_fast_execute_alu_at(base + 89,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 93,flags_,fExact_))
    return true;

  dsp_fast_execute_alu_at(base + 94,Y_,flags_,fExact_);

path_97:
  dsp_fast_execute_movereg_at(base + 97);
  dsp_fast_execute_movereg_at(base + 99);
  dsp_fast_execute_movereg_at(base + 101);
  dsp_fast_execute_alu_at(base + 103,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 106,Y_,flags_,fExact_);
  dsp_fast_execute_movereg_at(base + 111);
  dsp_fast_execute_alu_at(base + 113,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 116,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 118,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 121,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 123,flags_,fExact_))
    return true;

  dsp_fast_execute_alu_at(base + 124,Y_,flags_,fExact_);

path_127:
  dsp_fast_execute_alu_at(base + 127,Y_,flags_,fExact_);
  ...
  dsp_fast_execute_alu_at(base + 138,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 141,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 142);
  dsp_fast_control_target_at(base + 144);
  return true;
...
path_151:
  dsp_fast_execute_move_at(base + 151);
  dsp_fast_execute_move_at(base + 153);
  dsp_fast_execute_move_at(base + 155);
  dsp_fast_execute_movereg_at(base + 157);
  dsp_fast_execute_alu_at(base + 161,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 164,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 168,flags_,fExact_))
    return true;

  dsp_fast_execute_alu_at(base + 169,Y_,flags_,fExact_);

path_172:
  dsp_fast_execute_movereg_at(base + 172);
  dsp_fast_execute_movereg_at(base + 174);
  dsp_fast_execute_movereg_at(base + 176);
  dsp_fast_execute_alu_at(base + 178,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 181,Y_,flags_,fExact_);
  dsp_fast_execute_movereg_at(base + 186);
  dsp_fast_execute_alu_at(base + 188,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 191,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 193,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 196,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 198,flags_,fExact_))
    return true;
...
path_202:
  dsp_fast_execute_alu_at(base + 202,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 204,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 206,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 208,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 213,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 216,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 217);
  dsp_fast_control_target_at(base + 219);
  return true;

path_220:
  dsp_fast_execute_alu_at(base + 220,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 223,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 224);

path_226:
  dsp_fast_execute_move_at(base + 226);
                      ...
                      bool            *work_)
{
  ...
  dsp_fast_execute_move_at(base + 46);
  dsp_fast_execute_move_at(base + 48);
  dsp_fast_control_target_at(base + 50);
  return true;

path_51:
  if(dsp_fast_branch_taken_at(base + 51,flags_,fExact_))
    return true;

  dsp_fast_execute_move_at(base + 52);
  dsp_fast_execute_move_at(base + 54);
  dsp_fast_execute_move_at(base + 56);
  dsp_fast_control_target_at(base + 59);
  return true;

path_60:
  dsp_fast_execute_movereg_at(base + 60);
  dsp_fast_execute_alu_at(base + 62,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 66,flags_,fExact_))
    return true;

path_67:
  dsp_fast_execute_alu_at(base + 67,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 70,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 71);
  dsp_fast_execute_alu_at(base + 74,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 78,flags_,fExact_))
    return true;

path_79:
  dsp_fast_execute_alu_at(base + 79,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 82,flags_,fExact_))
    return true;

  dsp_fast_execute_move_at(base + 83);
  dsp_fast_execute_alu_at(base + 86,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 90,Y_,flags_,fExact_);
  dsp_fast_control_target_at(base + 94);
  return true;

path_95:
  dsp_fast_execute_move_at(base + 95);

path_97:
  dsp_fast_execute_alu_at(base + 97,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 101,flags_,fExact_))
    return true;

  dsp_fast_execute_alu_at(base + 102,Y_,flags_,fExact_);

path_105:
  dsp_fast_execute_movereg_at(base + 105);
  dsp_fast_execute_movereg_at(base + 107);
  dsp_fast_execute_movereg_at(base + 109);
  dsp_fast_execute_alu_at(base + 111,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 114,Y_,flags_,fExact_);
  dsp_fast_execute_movereg_at(base + 119);
  dsp_fast_execute_alu_at(base + 121,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 124,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 126,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 129,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 131,flags_,fExact_))
    return true;

  dsp_fast_execute_alu_at(base + 132,Y_,flags_,fExact_);

path_135:
  dsp_fast_execute_alu_at(base + 135,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 137,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 139,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 141,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 146,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 149,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 150);
  dsp_fast_control_target_at(base + 152);
  return true;

path_153:
  dsp_fast_execute_alu_at(base + 153,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 156,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 157);

path_159:
  dsp_fast_execute_move_at(base + 159);
  dsp_fast_execute_move_at(base + 161);
  dsp_fast_execute_move_at(base + 163);
  dsp_fast_execute_movereg_at(base + 165);
  dsp_fast_execute_alu_at(base + 169,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 172,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 176,flags_,fExact_))
    return true;

  dsp_fast_execute_alu_at(base + 177,Y_,flags_,fExact_);

path_180:
  dsp_fast_execute_movereg_at(base + 180);
  dsp_fast_execute_movereg_at(base + 182);
  dsp_fast_execute_movereg_at(base + 184);
  dsp_fast_execute_alu_at(base + 186,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 189,Y_,flags_,fExact_);
  dsp_fast_execute_movereg_at(base + 194);
  dsp_fast_execute_alu_at(base + 196,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 199,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 201,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 204,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 206,flags_,fExact_))
    return true;
...
path_210:
  ...
  dsp_fast_execute_alu_at(base + 212,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 214,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 216,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 221,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 224,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 225);
  dsp_fast_control_target_at(base + 227);
  return true;

path_228:
  dsp_fast_execute_alu_at(base + 228,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 231,flags_,fExact_))
    return true;

  dsp_fast_execute_movereg_at(base + 232);

path_234:
  dsp_fast_execute_move_at(base + 234);
  dsp_fast_execute_move_at(base + 236);
  dsp_fast_execute_move_at(base + 238);
  dsp_fast_execute_move_at(base + 240);
  dsp_fast_execute_move_at(base + 242);
                         ...
                         bool            *work_)
{
  ...
  *Y_              = (y << 1);
  ...
  jump.raw = DSP.NMem[base + 15];
  DSP.dregs.PC = jump.cif.BCH_ADDR;

  goto multiply_tail;

positive_ramp:
  imm.raw = DSP.NMem[base + 17];
...
multiply_tail:
  ...
  DSP.dregs.PC = base + DSP_TRIANGLE_LFO_23_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
...
first_tail_subtract:
  src1.raw = DSP.NMem[base + 16];
  src2.raw = DSP.NMem[base + 17];
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[base + 19];
  src2.raw = DSP.NMem[base + 20];
  ...
  *Y_              = y;
  ...
  src.raw = DSP.NMem[base + 22];
  dst.raw = DSP.NMem[base + 23];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 21]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 21]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  DSP.dregs.PC = base + DSP_VARMONO16_26_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
...
second_move:
  dst.raw = DSP.NMem[base + 14];
  src.raw = DSP.NMem[base + 15];
...
first_tail_subtract:
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[base + 20];
  src2.raw = DSP.NMem[base + 21];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + 19]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + 19]].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  DSP.dregs.PC = base + DSP_VARMONO16_27_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                     ...
                     bool            *work_)
{
  ...
  if(dsp_fast_branch_taken_at(base + 3,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x13))
        goto path_19;
      return true;
    }
  if(dsp_fast_branch_taken_at(base + 4,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x26))
        goto path_38;
      return true;
    }
...
path_12:
  ...
  inst.raw = DSP.NMem[base + 18];
  DSP.dregs.PC = inst.cif.BCH_ADDR;
  if(DSP.dregs.PC == (base + 0x23))
    goto path_35;
  return true;
...
path_19:
  ...
  dsp_fast_execute_alu_at(base + 24,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 27,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x23))
        goto path_35;
      return true;
...
path_38:
  ...
  dsp_fast_execute_alu_at(base + 45,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_VARMONO8_50_WORDS;
...
path_6:
  dsp_fast_execute_alu_at(base + 6,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 8,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x13))
        goto path_19;
...
path_37:
  inst.raw = DSP.NMem[base + 37];
  DSP.dregs.PC = base + 38;
  val = dsp_operand_load1();
  addr = inst.cif.BCH_ADDR;
  if(inst.nrof.DI)
    addr = dsp_read(addr);
  ...
  inst.raw = DSP.NMem[base + 49];
  DSP.dregs.PC = inst.cif.BCH_ADDR;
  if(DSP.dregs.PC == (base + 0x35))
    goto path_53;
  return true;
...
path_62:
  dsp_fast_execute_alu_at(base + 62,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_VARMONO8_67_WORDS;
...
path_232:
  dsp_fast_execute_alu_at(base + 232,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 236,Y_,flags_,fExact_);
...
path_240:
  dsp_fast_execute_alu_at(base + 240,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 244,Y_,flags_,fExact_);
                              ...
                              bool            *work_)
{
  ...
  dsp_fast_execute_alu_at(base + 0,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 2,flags_,fExact_))
    return true;

  inst.raw = DSP.NMem[base + 3];
  DSP.RBASEx4 = ((inst.cif.BCH_ADDR & 0x3F) << 2);
...
path_19:
  dsp_fast_execute_alu_at(base + 19,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 22,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 24,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x1C))
        goto path_28;
      return true;
  ...
  DSP.dregs.PC = inst.cif.BCH_ADDR;
  if(DSP.dregs.PC == (base + 0x42))
    goto path_66;
  return true;

path_28:
  dsp_fast_execute_alu_at(base + 28,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 31,flags_,fExact_))
    {
  ...
  dsp_fast_execute_alu_at(base + 32,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 35,flags_,fExact_))
    {
...
path_43:
  dsp_fast_execute_alu_at(base + 43,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 46,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x34))
        goto path_52;
      return true;
    }
...
path_52:
  dsp_fast_execute_alu_at(base + 52,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 54,Y_,flags_,fExact_);

path_55:
  inst.raw = DSP.NMem[base + 55];
  *RBSR_       = base + 56;
  DSP.dregs.PC = inst.cif.BCH_ADDR;
  return true;
...
path_66:
  dsp_fast_execute_alu_at(base + 66,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_ADPCMHALFMONO_71_WORDS;
                         ...
                         bool            *work_)
{
  ...
  inst.raw = DSP.NMem[base + 16];
  DSP.dregs.PC = inst.cif.BCH_ADDR;
  return true;

path_17:
  dsp_fast_execute_alu_at(base + 17,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 20,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x3A))
        goto path_58;
      return true;
  ...
  dsp_fast_execute_alu_at(base + 21,Y_,flags_,fExact_);

  inst.raw = DSP.NMem[base + 24];
  DSP.dregs.PC = base + 25;
  val = dsp_operand_load1();
  addr = inst.cif.BCH_ADDR;
  if(inst.nrof.DI)
    addr = dsp_read(addr);
  ...
  dsp_fast_execute_alu_at(base + 26,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 29,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 31,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x2B))
...
path_58:
  ...
  DSP.dregs.PC = base + DSP_ADPCMVARMONO_70_WORDS;
...
path_22:
  dsp_fast_execute_alu_at(base + 22,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 25,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x37))
        goto path_55;
      return true;
...
path_55:
  ...
  dsp_fast_execute_alu_at(base + 62,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 65,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 69,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 72,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_ADPCMVARSTEREO_77_WORDS;
...
path_21:
  dsp_fast_execute_alu_at(base + 21,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 22,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x19))
        goto path_25;
      return true;
    }
...
path_39:
  ...
  inst.raw = DSP.NMem[base + 47];
  DSP.dregs.PC = base + 48;
  val = dsp_operand_load1();
  addr = inst.cif.BCH_ADDR;
  if(inst.nrof.DI)
    addr = dsp_read(addr);
...
path_51:
  dsp_fast_execute_alu_at(base + 51,Y_,flags_,fExact_);

path_53:
  dsp_fast_execute_alu_at(base + 53,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_DCSQXDHALFMONO_59_WORDS;
                           ...
                           bool            *work_)
{
  ...
  dsp_fast_execute_alu_at(base + 3,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 7,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 9,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x36))
        goto path_54;
      return true;
    }

  dsp_fast_execute_alu_at(base + 10,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 13,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x15))
        goto path_21;
      return true;
  ...
  inst.raw = DSP.NMem[base + 20];
  DSP.dregs.PC = inst.cif.BCH_ADDR;
  if(DSP.dregs.PC == (base + 0x18))
    goto path_24;
  return true;
...
path_56:
  dsp_fast_execute_alu_at(base + 56,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_DCSQXDHALFMONO_62_WORDS;
                             ...
                             bool            *work_)
{
  ...
  inst.raw = DSP.NMem[base + 12];
  DSP.dregs.PC = inst.cif.BCH_ADDR;
  return true;

path_13:
  dsp_fast_execute_alu_at(base + 13,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 16,flags_,fExact_))
    return true;
  ...
  dsp_fast_execute_alu_at(base + 23,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 26,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 28,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x1F))
        goto path_31;
      return true;
    }

  dsp_fast_execute_alu_at(base + 30,Y_,flags_,fExact_);

path_31:
  dsp_fast_execute_alu_at(base + 31,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 34,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 37,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x2A))
        goto path_42;
      return true;
...
path_45:
  dsp_fast_execute_alu_at(base + 46,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 49,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 50,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 53,Y_,flags_,fExact_);

  inst.raw = DSP.NMem[base + 56];
  DSP.dregs.PC = base + 57;
  val = dsp_operand_load1();
  addr = inst.cif.BCH_ADDR;
  if(inst.nrof.DI)
    addr = dsp_read(addr);
...
path_63:
  ...
  dsp_fast_execute_alu_at(base + 66,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 69,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x4A))
        goto path_74;
      return true;
...
path_77:
  dsp_fast_execute_alu_at(base + 78,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 81,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 82,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 85,Y_,flags_,fExact_);

  inst.raw = DSP.NMem[base + 88];
  DSP.dregs.PC = base + 89;
  val = dsp_operand_load1();
  addr = inst.cif.BCH_ADDR;
  if(inst.nrof.DI)
    addr = dsp_read(addr);
  dsp_write(addr,val);
  DSP.dregs.PC = base + DSP_DCSQXDHALFSTEREO_92_WORDS;
...
path_36:
  dsp_fast_execute_alu_at(base + 37,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_DCSQXDMONO_42_WORDS;
                         ...
                         bool            *work_)
{
  ...
  dsp_fast_execute_alu_at(base + 10,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 13,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 15,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x12))
        goto path_18;
      return true;
...
path_18:
  ...
  inst.raw = DSP.NMem[base + 28];
  DSP.dregs.PC = inst.cif.BCH_ADDR;
  if(DSP.dregs.PC == (base + 0x20))
    goto path_32;
  return true;
...
path_55:
  dsp_fast_execute_alu_at(base + 56,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_DCSQXDSTEREO_61_WORDS;
...
path_59:
  DSP.dregs.PC = base + DSP_DCSQXDSTEREO_64_WORDS;
...
path_30:
  dsp_fast_execute_alu_at(base + 30,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 33,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 36,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x29))
        goto path_41;
      return true;
...
path_44:
  ...
  dsp_fast_execute_alu_at(base + 48,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 51,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_DCSQXDVARMONO_56_WORDS;
                        ...
                        bool            *work_)
{
  ...
  dsp_fast_execute_alu_at(base + 7,Y_,flags_,fExact_);
  dsp_fast_execute_alu_at(base + 10,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 12,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x10))
        goto path_16;
      return true;
                     ...
                     bool            *work_)
{
  ...
  branch.raw = DSP.NMem[base + 3];
  jump.raw   = DSP.NMem[base + 14];
  if((branch.cif.BCH_ADDR != ((base + 0x10) & 0x3FF)) ||
     (jump.cif.BCH_ADDR   != ((base + 0x17) & 0x3FF)))
    return false;
...
reset_path:
  ...
  *Y_ = y;
  ...
  inst.raw = DSP.NMem[base + 19];
  DSP.dregs.PC = base + 21;
  dsp_write(inst.cif.BCH_ADDR,0);

  inst.raw    = DSP.NMem[base + 21];
  operand.raw = DSP.NMem[base + 22];
  DSP.dregs.PC = base + 23;
  val = dsp_read(operand.nrof.OP_ADDR);
  dsp_write(inst.cif.BCH_ADDR,val);

final_move:
  inst.raw    = DSP.NMem[base + 23];
  operand.raw = DSP.NMem[base + 24];
  DSP.dregs.PC = base + DSP_ENVELOPE_27_WORDS;
  val = dsp_read(operand.nrof.OP_ADDR);
  dsp_write(inst.cif.BCH_ADDR,val);
...
final_write:
  ...
  DSP.dregs.PC = base + DSP_ENVFOLLOWER_15_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                          ...
                          bool            *work_)
{
  ...
  src1.raw = DSP.NMem[base + 2];
  src2.raw = DSP.NMem[base + 3];
  dst.raw  = DSP.NMem[base + 4];
  ...
  *Y_ = y;
  ...
  DSP.dregs.PC = base + DSP_FILTEREDNOISE_22_WORDS;
                       ...
                       bool            *work_)
{
  ...
  jump.raw = DSP.NMem[base + 10];
  if(jump.cif.BCH_ADDR != ((base + 0x10) & 0x3FF))
    return false;
...
product_tail:
  ...
  DSP.dregs.PC = base + DSP_FIXEDMONO8_21_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
                            ...
                            bool            *work_)
{
  ...
  *Y_              = y;
  ...
  DSP.dregs.PC = base + DSP_FIXEDMONOSAMPLE_10_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
...
static
void
dsp_fast_fixedstereo16swap_38_channel(uint32_t const  base,
                                      uint32_t const  off,
                                      uint32_t       *Y_,
                                      dsp_alu_flags_t *flags_,
                                      int             *fExact_)
{
  ...
  src.raw = DSP.NMem[base + off + 1];
  dst.raw = DSP.NMem[base + off + 2];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + off + 0]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + off + 0]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + off + 2;
  DSP.flags.WRITEBACK = src.nrof.OP_ADDR;
  DSP.flags.ALU1 = dsp_read(DSP.flags.WRITEBACK);

  DSP.dregs.PC = base + off + 3;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
  ...
  *Y_              = y;
  ...
  imm.raw = DSP.NMem[base + off + 4];
  dst.raw = DSP.NMem[base + off + 5];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + off + 3]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + off + 3]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + off + 5;
  DSP.flags.ALU1 = (uint16_t)(imm.iof.IMMEDIATE << (imm.iof.JUSTIFY & 3));

  DSP.dregs.PC = base + off + 6;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
  ...
  *Y_              = (y >> 8);
  ...
  src.raw = DSP.NMem[base + off + 7];
  imm.raw = DSP.NMem[base + off + 8];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + off + 6]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + off + 6]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + off + 8;
  DSP.flags.WRITEBACK = src.nrof.OP_ADDR;
  DSP.flags.ALU1 = dsp_read(DSP.flags.WRITEBACK);

  DSP.dregs.PC = base + off + 9;
  DSP.flags.ALU2 = (uint16_t)(imm.iof.IMMEDIATE << (imm.iof.JUSTIFY & 3));
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = (y << 8);
  ...
  src.raw = DSP.NMem[base + off + 10];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + off + 9]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + off + 9]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + off + 11;
  DSP.flags.WRITEBACK = src.nrof.OP_ADDR;
  DSP.flags.ALU1 = dsp_read(DSP.flags.WRITEBACK);
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  src.raw = DSP.NMem[base + off + 12];
  dst.raw = DSP.NMem[base + off + 13];

  DSP.flags.req.raw   = DSP.INSTTRAS[DSP.NMem[base + off + 11]].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[DSP.NMem[base + off + 11]].BS;
  DSP.flags.WRITEBACK = 0;

  DSP.dregs.PC = base + off + 13;
  DSP.flags.WRITEBACK = src.nrof.OP_ADDR;
  DSP.flags.MULT1 = dsp_read(DSP.flags.WRITEBACK);

  DSP.dregs.PC = base + off + 14;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
...
static
bool
dsp_fast_fixedstereo16swap_tail(uint32_t const  base,
                                uint32_t const  words,
                                bool const      direct_outputs,
                                uint32_t       *Y_,
                                dsp_alu_flags_t *flags_,
                                int             *fExact_)
{
  ...
  *Y_              = y;
  ...
  dsp_fast_fixedstereo16swap_38_channel(base,6,Y_,flags_,fExact_);

  DSP.dregs.PC = base + 21;

  dsp_fast_fixedstereo16swap_38_channel(base,21,Y_,flags_,fExact_);
  ...
  if(direct_outputs)
    {
      ...
      move.raw = DSP.NMem[base + 36];
      src.raw  = DSP.NMem[base + 37];

      DSP.dregs.PC = base + 38;
      val = dsp_read(src.nrof.OP_ADDR);
      dsp_write(move.cif.BCH_ADDR,val);

      move.raw = DSP.NMem[base + 38];
      src.raw  = DSP.NMem[base + 39];

      DSP.dregs.PC = base + 40;
      val = dsp_read(src.nrof.OP_ADDR);
      dsp_write(move.cif.BCH_ADDR,val);
                                  ...
                                  bool            *work_)
{
  ...
  *Y_              = y;
  ...
  return dsp_fast_fixedstereo16swap_tail(base,
                                         DSP_FIXEDSTEREO16SWAP_38_2DI_WORDS,
                                         false,
                                         Y_,flags_,fExact_);
}
                                  ...
                                  bool            *work_)
{
  ...
  *Y_              = y;
  ...
  return dsp_fast_fixedstereo16swap_tail(base,
                                         DSP_FIXEDSTEREO16SWAP_38_4BF_WORDS,
                                         false,
                                         Y_,flags_,fExact_);
}
                              ...
                              bool            *work_)
{
  ...
  *Y_              = y;
  ...
  return dsp_fast_fixedstereo16swap_tail(base,
                                         DSP_FIXEDSTEREO16SWAP_42_WORDS,
                                         true,
                                         Y_,flags_,fExact_);
}
                              ...
                              int             *fExact_)
{
  ...
  *Y_              = y;
  ...
  y = (*Y_ & ((uint32_t)(uint16_t)DSP.flags.ALU1 << 16));
  ...
  *Y_              = y << 8;
  ...
  DSP.dregs.PC = base + DSP_FIXEDSTEREO8_15_WORDS;
  DSP.flags.WRITEBACK = dst.nrof.OP_ADDR;
  (void)dsp_read(DSP.flags.WRITEBACK);
...
static
void
dsp_fast_fixedstereosample_16_run(uint32_t const  base,
                                  uint32_t       *Y_,
                                  dsp_alu_flags_t *flags_,
                                  int             *fExact_)
{
  ...
  *Y_              = y;
  ...
  src1.raw = DSP.NMem[base + 7];
  src2.raw = DSP.NMem[base + 8];
  dst.raw  = DSP.NMem[base + 9];
                                  ...
                                  bool            *work_)
{
  ...
  dsp_fast_fixedstereosample_16_run(base,Y_,flags_,fExact_);
...
path_29:
  dsp_fast_execute_alu_at(base + 29,Y_,flags_,fExact_);
  if(dsp_fast_branch_taken_at(base + 32,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x28))
        goto path_40;
      return true;
...
tail_43:
  dsp_fast_execute_alu_at(base + 43,Y_,flags_,fExact_);
  DSP.dregs.PC = base + DSP_HALFMONO8_49_WORDS;
...
static
void
dsp_fast_halfmonosample_23_tail(uint32_t const  base,
                                uint32_t       *Y_,
                                dsp_alu_flags_t *flags_,
                                int             *fExact_)
{
  ...
  *Y_              = y;
  ...
  flags_->negative = ((y >> 31) ? 1 : 0);
  *fExact_         = ((y & 0x0000F000) ? 0 : 1);
  *Y_              = ((uint32_t)((int32_t)y >> 1) & ALUSIZEMASK);
  ...
  DSP.dregs.PC = base + 16;
  DSP.flags.MULT2 = (uint16_t)(imm.iof.IMMEDIATE << (imm.iof.JUSTIFY & 3));
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_ = y;
  ...
  mov.raw = DSP.NMem[base + 19];
  src.raw = DSP.NMem[base + 20];

  DSP.dregs.PC = base + 21;
  val = dsp_read(src.nrof.OP_ADDR);
  dsp_write(mov.cif.BCH_ADDR,val);
}
                             ...
                             bool            *work_)
{
  ...
  if(pc == (base + 0x29))
    {
      DSP.dregs.PC = base + DSP_HALFSTEREOSAMPLE_44_WORDS;
      return true;
    }
  if(pc == (base + 0x15))
    goto tail;
                     ...
                     bool            *work_)
{
  ...
  inst.raw    = DSP.NMem[pc + 5];
  operand.raw = DSP.NMem[pc + 6];
  DSP.dregs.PC = pc + 7;
  val = dsp_read(operand.nrof.OP_ADDR);
  dsp_write(inst.cif.BCH_ADDR,val);

  inst.raw    = DSP.NMem[pc + 7];
  operand.raw = DSP.NMem[pc + 8];
  DSP.flags.req.raw   = DSP.INSTTRAS[inst.raw].req.raw;
  DSP.flags.BS        = DSP.INSTTRAS[inst.raw].BS;
  DSP.flags.WRITEBACK = 0;
  ...
  *Y_              = y;
  ...
  inst.raw    = DSP.NMem[pc + 28];
  operand.raw = DSP.NMem[pc + 29];
  DSP.dregs.PC = pc + DSP_HEAD_32_394_WORDS;
  val = dsp_read(operand.nrof.OP_ADDR);
  dsp_write(inst.cif.BCH_ADDR,val);
                     ...
                     bool            *work_)
{
  ...
  *Y_              = y;
  ...
  inst.raw    = DSP.NMem[pc + 28];
  operand.raw = DSP.NMem[pc + 29];
  DSP.dregs.PC = pc + DSP_HEAD_32_VD8_WORDS;
  val = dsp_read(operand.nrof.OP_ADDR);
  dsp_write(inst.cif.BCH_ADDR,val);
...
tail_move:
  ...
  DSP.dregs.PC = base + DSP_IMPULSE_12_WORDS;
  val = dsp_read(tail_src.nrof.OP_ADDR);
  dsp_write(tail_dst.cif.BCH_ADDR,val);
                    ...
                    bool            *work_)
{
  ...
  *Y_              = y;
  ...
  jump.raw = DSP.NMem[base + 6];
  DSP.dregs.PC = jump.cif.BCH_ADDR;

  return true;

tail_move:
  tail_dst.raw = DSP.NMem[base + 7];
  tail_src.raw = DSP.NMem[base + 8];

  DSP.dregs.PC = base + DSP_MAXIMUM_11_WORDS;
  val = dsp_read(tail_src.nrof.OP_ADDR);
  dsp_write(tail_dst.cif.BCH_ADDR,val);
...
tail_move:
  ...
  DSP.dregs.PC = base + DSP_MINIMUM_11_WORDS;
  val = dsp_read(tail_src.nrof.OP_ADDR);
  dsp_write(tail_dst.cif.BCH_ADDR,val);
                  ...
                  bool            *work_)
{
  ...
  if(dsp_fast_branch_taken_at(base + 0,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x0E))
        goto path_14;
      return true;
    }

  if(dsp_fast_branch_taken_at(base + 1,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x1F))
        goto tail_31;
      return true;
  ...
  dsp_fast_execute_alu_at(base + 2,Y_,flags_,fExact_);

  if(dsp_fast_branch_taken_at(base + 4,flags_,fExact_))
    {
      if(DSP.dregs.PC == (base + 0x08))
        goto path_8;
      return true;
    }
