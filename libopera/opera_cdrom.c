...
static void cdrom_restore_audio_callbacks(cdrom_device_t *cd_);
...
static
uint32_t
cdrom_data_audio_playback_lba(const cdrom_device_t *cd_,
                              uint64_t              cycles_)
{
...
cdrom_data_audio_clock_stop(cdrom_device_t *cd_)
{
  ...
  current_lba = cdrom_data_audio_playback_lba(cd_,opera_clock_cpu_get_cycles());
...
opera_cdrom_set_callbacks(opera_cdrom_get_size_cb_t    get_size_,
                          ...
                          opera_cdrom_audio_play_cb_t  audio_play_,
                          opera_cdrom_audio_stop_cb_t  audio_stop_)
{
  ...
  g_CDROM_CALLBACKS.audio_play  = audio_play_;
  g_CDROM_CALLBACKS.audio_stop  = audio_stop_;
}
...
static
void
cdrom_restore_audio_callbacks(cdrom_device_t *cd_)
{
  uint32_t start_lba;

  if(cd_->callbacks.audio_stop != NULL)
    cd_->callbacks.audio_stop();

  if(!g_CDROM_STATE.data.audio_clock_active ||
     (cd_->callbacks.audio_play == NULL) ||
     (cd_->blocks_requested != 0) ||
     (cd_->read_sector_size != CDROM_DA))
    return;

  start_lba = cdrom_data_audio_playback_lba(cd_,opera_clock_cpu_get_cycles());
  if(start_lba < cd_->current_sector)
    cd_->callbacks.audio_play(start_lba,cd_->current_sector);
}
                          ...
                          uint32_t        data_size_)
{
  ...
  opera_cdrom_restore_callbacks(cd_);
  cdrom_restore_audio_callbacks(cd_);
...
ode_read_file(cdrom_device_t *cd_)
{
  ...
  actual = 0;
  if(g_CDROM_STATE.ode.file != NULL)
    actual = (uint16_t)filestream_read(g_CDROM_STATE.ode.file,g_CDROM_STATE.ode.file_buffer,(int64_t)len);
...
ode_write_file(cdrom_device_t *cd_)
{
  ...
  actual = 0;
  if(g_CDROM_STATE.ode.file != NULL)
    actual = (uint16_t)filestream_write(g_CDROM_STATE.ode.file,g_CDROM_STATE.ode.file_buffer,(int64_t)len);
...
opera_cdrom_do_cmd(cdrom_device_t *cd_)
{
  ...
  switch(cd_->cmd[0])
    ...
    case CDROM_CMD_FLUSH:
      ...
      /*
        PLAY AUDIO MSF
        0E mm-ss-ff mm-ss-ff
        Play audio from start MSF to end MSF

        This path is mainly for command completeness. Normal 3DO software,
        including the CD player, does not use the drive's PLAY_AUDIO/CDDA
        playback path; it issues READ_DATA requests for audio sectors and
        sends the decoded stream to DSPP itself.
      */
    case CDROM_CMD_PLAY_AUDIO_MSF:
      {
        ...
        if((cd_->xbus_status & CDROM_STATUS_DOOR) &&
           ...
           (cd_->xbus_status & CDROM_STATUS_SPIN_UP))
          {
            ...
            if(cd_->callbacks.audio_play)
              cd_->callbacks.audio_play(start_lba,end_lba);
      ...
      /*
        PLAY AUDIO TRACK
        0F tt ii TT II 00
        Play audio from track tt to track TT
      */
    case CDROM_CMD_PLAY_AUDIO_TRACK:
      {
        ...
        /* end_track is the last track to play (inclusive).
           end_lba should be the start of the track after end_track,
           or the end of the disc if end_track is the last track. */
    ...
    case CDROM_CMD_DATA_PATH_CHECK:
      ...
      /*
        ...
        native tagged status 10 bytes:
        82 ee ee ee ee ee ee ee ee XS
        Portfolio reads the MEI error from actual-7.
      */
    case CDROM_CMD_GET_LAST_STATUS:
      ...
      /*
        read id
        native tagged status 12 bytes:
        83 ManuId[2] ManuDev[2] ManuRev[2] ManuFlags[2] RomTagWords[2] XS

        Portfolio issues READ_DEVICE_DRIVER only when DEVHAS_DRIVER (0x0500)
        is set in ManuFlags. Keep the driver flag and table size clear until
        Opera provides a real device-ROM driver stream for command 0x8E.
      */
    ...
    case CDROM_CMD_READ_SUBQ:
      if((cd_->xbus_status & CDROM_STATUS_DOOR) &&
         ...
         (cd_->xbus_status & CDROM_STATUS_SPIN_UP))
        {
          ...
          /* Sub-Q ADR/Control = swapped nibbles of TOC CDCTL */
    ...
    case CDROM_CMD_READ_DEVICE_DRIVER:
      ...
      /* FCare/Fixel ODE extended ID */
    case ODE_CMD_EXTENDED_ID:
      ode_extended_id(cd_);
      break;

      /* FCare/Fixel ODE TOC and playlist commands */
    case ODE_CMD_CHANGE_TOC:
      ode_change_toc(cd_);
      break;
    case ODE_CMD_READ_TOC_WINDOW:
    ...
    case ODE_CMD_READ_TOC_LIST:
      ...
      /* FCare/Fixel ODE file-buffer and file commands */
    case ODE_CMD_CREATE_FILE:
      ode_create_file(cd_);
      break;
    case ODE_CMD_OPEN_FILE:
