...
#include "deps/libchdr/include/libchdr/cdrom.h"
#endif
...
#ifdef HAVE_CHD
typedef struct cdimage_chd_metadata_s
{
  uint32_t track;
  uint32_t frames;
  uint32_t pregap;
  ...
  char     pgtype[64];
  char     pgsub[32];
} cdimage_chd_metadata_t;

static
int
cdimage_chd_get_metadata(chd_file                 *chd_,
                         int                       index_,
                         cdimage_chd_metadata_t   *metadata_)
{
  char meta[256];
  UINT32 meta_size;
  unsigned int track;
  unsigned int frames;
  unsigned int pregap;
  unsigned int postgap;
  chd_error meta_err;

  memset(metadata_,0,sizeof(*metadata_));

  meta_err = chd_get_metadata(chd_,CDROM_TRACK_METADATA2_TAG,index_,
                              meta,sizeof(meta),&meta_size,NULL,NULL);
  if(meta_err == CHDERR_NONE)
    {
      if(sscanf(meta,
                "TRACK:%u TYPE:%63s SUBTYPE:%31s FRAMES:%u "
                "PREGAP:%u PGTYPE:%63s PGSUB:%31s POSTGAP:%u",
                &track,
                metadata_->type,
                metadata_->subtype,
                &frames,
      ...
      metadata_->frames  = frames;
      metadata_->pregap  = pregap;
      metadata_->postgap = postgap;
      return 0;
    }

  meta_err = chd_get_metadata(chd_,CDROM_TRACK_METADATA_TAG,index_,
                              meta,sizeof(meta),&meta_size,NULL,NULL);
  if(meta_err != CHDERR_NONE)
    return -1;
...
static
bool
cdimage_chd_pregap_in_track(const cdimage_chd_metadata_t *metadata_)
{
...
static
uint32_t
cdimage_chd_data_frames(const cdimage_chd_metadata_t *metadata_)
{
  ...
  if(err == CHDERR_NONE)
    {
      ...
      for(i = 0; i < CDIMAGE_MAX_TRACKS; i++)
        {
          cdimage_chd_metadata_t metadata;
          uint32_t data_frames;

          if(cdimage_chd_get_metadata(chd,i,&metadata) < 0)
            break;
                          ...
                          size_t     bufsize_)
{
  ...
  if(cdimage_->num_files > 0)
    {
      uint8_t file_index = cdimage_->tracks[track].file_index;
      if(file_index >= cdimage_->num_files || cdimage_->files[file_index].fp == NULL)
        return -1;
      fp = cdimage_->files[file_index].fp;
    }
  if(fp == NULL)
    return -1;

  offset   = ((bufsize_ >= cdimage_->tracks[track].mode) ?
              0 : cdimage_->tracks[track].offset);
  bufsize_ = MIN(bufsize_, cdimage_->tracks[track].mode);
  pos      = (cdimage_->tracks[track].file_offset +
              ((sector_ - cdimage_->tracks[track].start_lba) *
               cdimage_->tracks[track].mode) +
              offset);
  ...
  /*
   * Raw BIN/CUE CD-DA sectors need native-order samples for both libretro
   * CDDA mixing and the 3DO READ_DATA audio path. CHD audio is already
   * decoded by libchdr in that order, so CHD images leave swap_audio clear.
   */
  if((rv > 0) &&
     (cdimage_->tracks[track].type == CDIMAGE_TRACK_AUDIO) &&
     cdimage_->swap_audio &&
     IS_LITTLE_ENDIAN)
    {
      ssize_t i;
      uint8_t *sector = (uint8_t*)buf_;

      for(i = 0; (i + 1) < rv; i += 2)
        {
          uint8_t tmp   = sector[i];
          sector[i]     = sector[i + 1];
          sector[i + 1] = tmp;
        }
    }
